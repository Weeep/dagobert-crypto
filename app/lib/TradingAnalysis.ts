...
export class TradingAnalysis {
  ...
  getRsi4(period: number): number | null {
    ...
    for (let i = 1; i < rsiCandles.length; i++) {
      changes.push(new Big(rsiCandles[i].close).minus(rsiCandles[i - 1].close));
    }

    let gains: Big[] = changes.map((change) =>
      change.gt(0) ? change : new Big(0)
    );
    let losses: Big[] = changes.map((change) =>
      change.lt(0) ? change.abs() : new Big(0)
    );

    // Compute RMA (Wilder's smoothing) for gains and losses
    let avgGain = gains
      .slice(0, period)
      .reduce((acc, val) => acc.plus(val), new Big(0))
      .div(period);
  ...
  getRsi3(period: number): number | null {
    if (this.candles.length < period + 1) return null;
    ...
    // Compute initial RMA (Wilder's smoothing method) of gains and losses
    let avgGain = gains
      .slice(0, period)
      .reduce((acc, val) => acc.plus(val), new Big(0))
      .div(period);
  ...
  getRsi_ooooooooooooooo(period: number, index: number = 0): number | null {
    if (index > 0) {
      console.error("index must be 0 or negative integer");
      return null;
    }
