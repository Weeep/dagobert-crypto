...
const TransactionCardContainer: React.FC<Props> = ({ transactions }) => {
    ...
    <div className="container mx-auto p-8">
        ...
        <h1 className="text-2xl font-bold mb-4">Transaction Cards</h1>
        <div id="cont" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {(filteredData.length !== 0) && filteredData.map(transaction => <TransactionCard key={transaction.orderId} transaction={transaction} />)}
        </div>
    </div>
  );
};
