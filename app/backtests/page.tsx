      ...
      <div className="mt-8 space-y-5">{strategies.map((strategy) => <section key={strategy.id} className="overflow-hidden rounded-2xl border border-emerald-900/70 bg-slate-900">
        ...
        <div className="divide-y divide-slate-800">{strategy.runs.map((run) => <details key={run.id} className="group">
          <summary className="cursor-pointer list-none px-5 py-4 hover:bg-slate-800/60"><div className="flex flex-wrap items-center justify-between gap-3">
            <div><span className="font-semibold">{run.bot.name}</span><span className="ml-3 text-sm text-slate-400">{run.bot.pair} · {run.bot.timeframe}</span>
              <p className="mt-1 text-xs text-slate-500">Run at {dateTime(run.startedAt)} · range {dateTime(run.from)} – {dateTime(run.to)}</p></div>
