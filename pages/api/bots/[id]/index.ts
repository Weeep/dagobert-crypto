...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "DELETE") {
    const result = await tradingBotUseCases.deleteBot.execute(userId, id);
    return result.ok ? res.status(204).end() : res.status(result.error === "Bot not found" ? 404 : 409)
      .json({ error: { code: result.error === "Bot not found" ? "NOT_FOUND" : "INVALID_STATE", message: result.error } });
  }
  if (req.method !== "PATCH") { res.setHeader("Allow", "GET, PATCH, DELETE"); return res.status(405).end(); }
  ...
  if (body.archived !== undefined) input.archived = body.archived === true;
